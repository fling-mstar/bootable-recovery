...
Value* DeletePartitionFileFn(const char* name, State* state, int argc, Expr* argv[]) {
    char* result = NULL;
    if (argc != 2) {
        return ErrorAbort(state, "%s() expects 2 args, got %d", name, argc);
    }
    ...
    char* mount_point;
    char* file_path;
    if (ReadArgs(state, argv, 2, &mount_point, &file_path) < 0) {
        return NULL;
    }
    ...
    if (strlen(file_path) == 0) {
        ErrorAbort(state, "file_path argument to %s() can't be empty", name);
        goto done;
    }
    ...
    sprintf(buffer, "%s/%s", mount_point,file_path);
    dirUnlinkHierarchy(buffer);
...
Value* UpdateMbootFn(const char* name, State* state, int argc, Expr* argv[]) {
    char** args = ReadVarArgs(state, argc, argv);
    if (args == NULL) {
        return NULL;
    }
    ...
    char* line = strtok(buffer, "\n");
    if (NULL == line) {
        fprintf(((UpdaterInfo*)(state->cookie))->cmd_pipe, "update_mboot %s\n", line);
    } else {
        while (line) {
            fprintf(((UpdaterInfo*)(state->cookie))->cmd_pipe,"update_mboot %s\n", line);
            line = strtok(NULL, "\n");
...
Value* ReloadEnvFn(const char* name, State* state, int argc, Expr* argv[]) {
    char** args = ReadVarArgs(state, argc, argv);
    if (args == NULL) {
        return NULL;
    }
    ...
    fprintf(((UpdaterInfo*)(state->cookie))->cmd_pipe, "reload_env\n");

    return StringValue(buffer);
}

Value* ReloadPanelFn(const char* name, State* state, int argc, Expr* argv[]) {
    char** args = ReadVarArgs(state, argc, argv);
    if (args == NULL) {
        return NULL;
    }
    ...
    fprintf(((UpdaterInfo*)(state->cookie))->cmd_pipe, "reload_panel\n");


    return StringValue(buffer);
}

Value* UpdateSecureInfoFn(const char* name, State* state, int argc, Expr* argv[]) {
    if (argc != 2) {
        return ErrorAbort(state, "%s() expects 2 args, got %d", name, argc);
    }
    char* partition;
    char* filename;
    if (ReadArgs(state, argv, 2, &partition, &filename) < 0) {
        return NULL;
    }
    fprintf(((UpdaterInfo*)(state->cookie))->cmd_pipe, "update_secure_info %s %s\n",partition ,filename);
    free(partition);
    return StringValue(filename);
}

Value* UpdateNuttxConfigFn(const char* name, State* state, int argc, Expr* argv[]) {
    if (argc != 2) {
        return ErrorAbort(state, "%s() expects 2 args, got %d", name, argc);
    }
    char* config;
    char* filename;
    if (ReadArgs(state, argv, 2, &config, &filename) < 0) {
        return NULL;
    }
    fprintf(((UpdaterInfo*)(state->cookie))->cmd_pipe, "update_nuttx_config %s %s\n",config ,filename);
    free(config);
    return StringValue(filename);
}
Value* UpdateUrsaFn(const char* name, State* state, int argc, Expr* argv[]) {
    if (argc != 1) {
        return ErrorAbort(state, "%s() expects 1 args, got %d", name, argc);
    }
    char* filename;
    ...
    fprintf(((UpdaterInfo*)(state->cookie))->cmd_pipe, "update_ursa %s\n",filename);
    return StringValue(filename);
}
...
void RegisterInstallFunctions() {
    ...
    // MStar Android Patch Begin
    RegisterFunction("delete_partition_file", DeletePartitionFileFn);
    RegisterFunction("update_mboot", UpdateMbootFn);
    RegisterFunction("reload_env", ReloadEnvFn);
    RegisterFunction("reload_panel", ReloadPanelFn);
    RegisterFunction("update_secure_info", UpdateSecureInfoFn);
    RegisterFunction("update_nuttx_config", UpdateNuttxConfigFn);
    RegisterFunction("update_ursa",UpdateUrsaFn);
    // MStar Android Patch End
}
