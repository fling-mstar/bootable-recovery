...
static const struct option OPTIONS[] = {
  ...
  { "restore_system", no_argument, NULL, 'r' },
...
storage_update(int* wipe_cache, Device* device)
{
    ...
    do {
        ...
        if (chosen_item == 0) {          // item 0 is always "../"
            ...
            ret = update_directory(new_path, NULL, wipe_cache, device);
            if (ret >= 0) break;
        }
    } while (true);
...
reset_reproducerate() {
    ...
    char *tmp;
    if (-1 == envutils_init()) {
        printf("init env in emmc failed!\n");
        return -1;
    }
    int length = BOOTARGS_LENGTH;
    int len = 0;
    char bootargs[length + 1];
    memset(bootargs, 0, length + 1);

    if (0 == envutils_get("bootargs", bootargs, length)) {
    ...
    if (NULL == (rate = strstr(bootargs, "rate"))) {
        printf("there is no rate in bootargs!\n");
        return 0;
    }
    ...
    while (NULL != rate) {
        ...
        if (0 == memcmp(rate, "rate2160", len)) {
            if (-1 == envutils_set("reproducerate2160","")) {
                return -1;
            }
            if (-1 == envutils_set2bootargs("rate2160=","")) {
                return -1;
            }
        } else if (0 == memcmp(rate, "rate1080", len)) {
            if (-1 == envutils_set("reproducerate1080","")) {
                return -1;
            }
            if (-1 == envutils_set2bootargs("rate1080=","")) {
                return -1;
            }
        } else if (0 == memcmp(rate, "rate720", len)) {
            if (-1 == envutils_set("reproducerate720","")) {
                return -1;
            }
            if (-1 == envutils_set2bootargs("rate720=","")) {
                return -1;
            }
        } else if (0 == memcmp(rate, "rate576", len)) {
            if (-1 == envutils_set("reproducerate576","")) {
                return -1;
            }
            if (-1 == envutils_set2bootargs("rate576=","")) {
                return -1;
            }
        } else if (0 == memcmp(rate, "rate480", len)) {
            if (-1 == envutils_set("reproducerate480","")) {
                return -1;
            }
            if (-1 == envutils_set2bootargs("rate480=","")) {
                return -1;
            }
        }
        if (-1 == envutils_set2bootargs("reproducerate=", "")) {
            return -1;
        }
        if (-1 == envutils_set("resolution_reset", "4")) {
            return -1;
        }
...
wipe_data(int confirm, Device* device) {
    ...
    if (reset_tvdatabase() != 0) {
        ui->Print("reset factory db failed.\n");
    }
#ifdef USE_ENV_UTILS
    if (reset_reproducerate() != 0) {
        ui->Print("reset reproducerate failed.\n");
    }
#endif
    // MStar Android Patch End
    ...
    for (;;) {
        ...
        switch (chosen_item) {
            ...
            break;
                ...
                ret = mstaredison_system_backup(NULL);
                if (ret) {
                    ui->SetTipTitle(RecoveryUI::TIP_TITLE_ERROR);
           ...
           case Device::SYSTEM_RESTORE:
            ...
            break;
                write_bootloader_message(1);
                ret = mstaredison_system_restore(NULL,  device);
                if (ret) {
                    ui->SetTipTitle(RecoveryUI::TIP_TITLE_ERROR);
...
main(int argc, char **argv) {
    ...
    if (update_package != NULL) {
        ...
        if (wipe_data) {
            ...
            if (init_backup_recovery(NULL, 0) && mstaredison_system_backup("/sdcard/backup_system")) status = INSTALL_ERROR;
            if (status != INSTALL_SUCCESS) ui->Print("backup system failed.\n");
        }  else if (restore_system) {
            if (init_backup_recovery(NULL, 0)  && mstaredison_system_restore("/sdcard/backup_system/", NULL)) status = INSTALL_ERROR;
